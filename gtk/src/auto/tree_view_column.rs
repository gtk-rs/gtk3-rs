...
use crate::{
    ...
    TreeViewColumnSizing, Widget, ffi,
};
use glib::{
...
pub trait TreeViewColumnExt: IsA<TreeViewColumn> + 'static {
        ...
        unsafe extern "C" fn destroy_func(data: glib::ffi::gpointer) {
            unsafe {
                let _callback = Box_::from_raw(
                    data as *mut Option<
                        Box_<
                            dyn Fn(&TreeViewColumn, &CellRenderer, &TreeModel, &TreeIter) + 'static,
                        >,
                    >,
                );
            }
    ...
    fn connect_sort_column_id_notify<F: Fn(&Self) + 'static>(&self, f: F) -> SignalHandlerId {
        ...
        unsafe {
            ...
            connect_raw(
                self.as_ptr() as *mut _,
                c"notify::sort-column-id".as_ptr(),
                Some(std::mem::transmute::<*const (), unsafe extern "C" fn()>(
    ...
    fn connect_sort_indicator_notify<F: Fn(&Self) + 'static>(&self, f: F) -> SignalHandlerId {
        ...
        unsafe {
            ...
            connect_raw(
                self.as_ptr() as *mut _,
                c"notify::sort-indicator".as_ptr(),
                Some(std::mem::transmute::<*const (), unsafe extern "C" fn()>(
