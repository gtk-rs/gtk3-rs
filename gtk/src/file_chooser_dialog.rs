...
pub struct FileChooserDialogBuilder {
    ...
    buttons: Vec<(String, ResponseType)>,
}
...
impl FileChooserDialogBuilder {
    ...
    pub fn build(self) -> FileChooserDialog {
        ...
        if self.buttons.is_empty() {
            ...
            let buttons: Vec<_> = self.buttons.iter().map(|(s, r)| (s.as_str(), *r)).collect();
            let dialog = FileChooserDialog::with_buttons(
                None,
                None::<&Window>,
                self.action.unwrap_or(FileChooserAction::Open),
                &buttons,
            );
            dialog
                .set_properties(&properties)
                .expect("set_properties failed");
            dialog
        }
    }
    ...
    pub fn add_button(mut self, title: &str, response_type: ResponseType) -> Self {
        self.buttons.push((title.to_owned(), response_type));
        self
    }
}
