...
impl<O: IsA<DragContext>> DragContextExtManual for O {
    fn drag_get_selection(&self) -> Atom {
        unsafe { from_glib_none(ffi::gdk_drag_get_selection(self.as_ref().to_glib_none().0) as *mut _) }
    }

    fn drag_abort(&self, time_: u32) {
        unsafe { ffi::gdk_drag_abort(self.as_ref().to_glib_none().0, time_) }
    }

    fn drop_reply(&self, accepted: bool, time_: u32) {
        unsafe { ffi::gdk_drop_reply(self.as_ref().to_glib_none().0, accepted.to_glib(), time_) }
    }

    fn drop(&self, time_: u32) {
        unsafe { ffi::gdk_drag_drop(self.as_ref().to_glib_none().0, time_) }
    }
    ...
    fn drag_find_window_for_screen(&self, drag_window: &Window, screen: &Screen,
        ...
        unsafe {
            ...
            ffi::gdk_drag_find_window_for_screen(self.as_ref().to_glib_none().0,
    ...
    fn drag_motion(&self, dest_window: &Window, protocol: DragProtocol, x_root: i32,
        ...
        unsafe {
            from_glib(
                ffi::gdk_drag_motion(self.as_ref().to_glib_none().0, dest_window.to_glib_none().0, protocol.to_glib(), 
                    x_root, y_root, suggested_action.to_glib(), possible_actions.to_glib(), time_))
        }
    }

    fn drop_finish(&self, success: bool, time_: u32) {
        unsafe { ffi::gdk_drop_finish(self.as_ref().to_glib_none().0, success.to_glib(), time_) }
    }

    fn drag_status(&self, action: DragAction, time_: u32) {
        unsafe { ffi::gdk_drag_status(self.as_ref().to_glib_none().0, action.to_glib(), time_) }
    }

    fn drag_drop_succeeded(&self) -> bool {
        unsafe { from_glib(ffi::gdk_drag_drop_succeeded(self.as_ref().to_glib_none().0)) }
    }
    ...
    fn drag_drop_done(&self, success: bool) {
        skip_assert_initialized!();
        unsafe {
            ffi::gdk_drag_drop_done(self.as_ref().to_glib_none().0, success.to_glib());
        }
    }
}
