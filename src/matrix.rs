...
impl Matrix {
    ...
    pub fn new_from_vec4(v0: &Vec4, v1: &Vec4, v2: &Vec4, v3: &Vec4) -> Matrix {
        ...
        unsafe {
            let alloc = ffi::graphene_matrix_alloc();
            ffi::graphene_matrix_init_from_vec4(alloc, v0.to_glib_none().0, v1.to_glib_none().0, v2.to_glib_none().0, v3.to_glib_none().0);
            from_glib_full(alloc)
        }
    }
    ...
    pub fn new_look_at(eye: &Vec3, center: &Vec3, up: &Vec3) -> Matrix {
        ...
        unsafe {
            let alloc = ffi::graphene_matrix_alloc();
            ffi::graphene_matrix_init_look_at(alloc, eye.to_glib_none().0, center.to_glib_none().0, up.to_glib_none().0);
            from_glib_full(alloc)
        }
    }
