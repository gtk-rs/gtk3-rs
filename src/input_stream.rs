...
pub trait InputStreamExtManual {
    ...
    fn read_all<'a, B: AsMut<[u8]>, P: Into<Option<&'a Cancellable>>>(&self, buffer: B, cancellable: P) -> Result<usize, Error>;
...
impl<O: IsA<InputStream>> InputStreamExtManual for O {
    ...
    fn read_all<'a, B: AsMut<[u8]>, P: Into<Option<&'a Cancellable>>>(&self, mut buffer: B, cancellable: P) -> Result<usize, Error> {
        let cancellable = cancellable.into();
        let cancellable = cancellable.to_glib_none();
        let buffer = buffer.as_mut();
        let count = buffer.len() as usize;
        ...
        extern "C" fn read_all_async_trampoline(_source_object: *mut gobject_ffi::GObject, res: *mut ffi::GAsyncResult, user_data: glib_ffi::gpointer)
        {
            ...
            unsafe {
                ...
                let result = if error.is_null() {
                    assert!(vec.capacity() >= bytes_read as usize);
                    vec.set_len(bytes_read as usize);
                    Ok(vec)
                } else {
