...
impl AsRawFd for UnixInputStream {
    fn as_raw_fd(&self) -> RawFd {
        unsafe {
            gio_sys::g_unix_input_stream_get_fd(self.to_glib_none().0) as _
        }
    }
}
...
impl<O: IsA<UnixInputStream>> UnixInputStreamExtManual for O {
    ...
    fn get_fd<T: FromRawFd>(&self) -> T {
        unsafe {
            T::from_raw_fd(gio_sys::g_unix_input_stream_get_fd(self.as_ref().to_glib_none().0))
        }
    }

    unsafe fn set_close_fd(&self, close_fd: bool) {
        gio_sys::g_unix_input_stream_set_close_fd(self.as_ref().to_glib_none().0, close_fd.to_glib());
    }
}
