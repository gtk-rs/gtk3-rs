...
impl<O: IsA<FileOutputStream>> FileOutputStreamExt for O {
    ...
    fn query_info_async<'a, P: Into<Option<&'a Cancellable>>, Q: Fn(Result<FileInfo, Error>) + Send + Sync + 'static>(&self, attributes: &str, io_priority: i32, cancellable: P, callback: Q) {
        ...
        unsafe {
            ffi::g_file_output_stream_query_info_async(self.to_glib_none().0, attributes.to_glib_none().0, io_priority, cancellable.0, Some(callback), Box::into_raw(user_data) as *mut _);
        }
    }
}
