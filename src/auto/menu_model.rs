...
impl<O: IsA<MenuModel> + IsA<Object>> MenuModelExt for O {
    ...
    fn get_item_attribute_value<'a, P: Into<Option<&'a glib::VariantTy>>>(&self, item_index: i32, attribute: &str, expected_type: P) -> Option<glib::Variant> {
        ...
        unsafe {
            from_glib_full(ffi::g_menu_model_get_item_attribute_value(self.to_glib_none().0, item_index, attribute.to_glib_none().0, expected_type))
        }
    }
...
where P: IsA<MenuModel> {
    callback_guard!();
    let f: &Box_<Fn(&P, i32, i32, i32) + 'static> = transmute(f);
