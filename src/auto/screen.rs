...
impl Screen {
    ...
    pub fn get_monitor_at_window<P: IsA<Window>>(&self, window: &P) -> i32 {
        unsafe {
            ffi::gdk_screen_get_monitor_at_window(self.to_glib_none().0, window.as_ref().to_glib_none().0)
        }
    }
    ...
    //        gobject_ffi::g_object_get_property(self.as_ptr() as *mut gobject_ffi::GObject, b"font-options\0".as_ptr() as *const _, value.to_glib_none_mut().0);
    //        value.get().unwrap()
    //    }
    //}
