// This file was generated by gir (https://github.com/gtk-rs/gir)
// from gir-files (https://github.com/gtk-rs/gir-files)
// DO NOT EDIT

use Layer;
use Rectangle;
use ffi;
use glib;
use glib::object::Downcast;
use glib::object::IsA;
use glib::signal::SignalHandlerId;
use glib::signal::connect;
use glib::translate::*;
use glib_ffi;
use gobject_ffi;
use std::boxed::Box as Box_;
use std::mem;
use std::mem::transmute;
use std::ptr;

glib_wrapper! {
    pub struct Component(Object<ffi::AtkComponent, ffi::AtkComponentIface>);

    match fn {
        get_type => || ffi::atk_component_get_type(),
    }
}

pub trait ComponentExt {
    //#[cfg_attr(feature = "v2_9_4", deprecated)]
    //fn add_focus_handler(&self, handler: /*Unknown conversion*//*Unimplemented*/FocusHandler) -> u32;

    //fn contains(&self, x: i32, y: i32, coord_type: /*Ignored*/CoordType) -> bool;

    #[cfg(any(feature = "v1_12", feature = "dox"))]
    fn get_alpha(&self) -> f64;

    //fn get_extents(&self, coord_type: /*Ignored*/CoordType) -> (i32, i32, i32, i32);

    fn get_layer(&self) -> Layer;

    fn get_mdi_zorder(&self) -> i32;

    //fn get_position(&self, coord_type: /*Ignored*/CoordType) -> (i32, i32);

    fn get_size(&self) -> (i32, i32);

    fn grab_focus(&self) -> bool;

    //fn ref_accessible_at_point(&self, x: i32, y: i32, coord_type: /*Ignored*/CoordType) -> Option<Object>;

    #[cfg_attr(feature = "v2_9_4", deprecated)]
    fn remove_focus_handler(&self, handler_id: u32);

    //fn set_extents(&self, x: i32, y: i32, width: i32, height: i32, coord_type: /*Ignored*/CoordType) -> bool;

    //fn set_position(&self, x: i32, y: i32, coord_type: /*Ignored*/CoordType) -> bool;

    fn set_size(&self, width: i32, height: i32) -> bool;

    fn connect_bounds_changed<F: Fn(&Self, &Rectangle) + 'static>(&self, f: F) -> SignalHandlerId;
}

impl<O: IsA<Component> + IsA<glib::object::Object>> ComponentExt for O {
    //fn add_focus_handler(&self, handler: /*Unknown conversion*//*Unimplemented*/FocusHandler) -> u32 {
    //    unsafe { TODO: call ffi::atk_component_add_focus_handler() }
    //}

    //fn contains(&self, x: i32, y: i32, coord_type: /*Ignored*/CoordType) -> bool {
    //    unsafe { TODO: call ffi::atk_component_contains() }
    //}

    #[cfg(any(feature = "v1_12", feature = "dox"))]
    fn get_alpha(&self) -> f64 {
        unsafe {
            ffi::atk_component_get_alpha(self.to_glib_none().0)
        }
    }

    //fn get_extents(&self, coord_type: /*Ignored*/CoordType) -> (i32, i32, i32, i32) {
    //    unsafe { TODO: call ffi::atk_component_get_extents() }
    //}

    fn get_layer(&self) -> Layer {
        unsafe {
            from_glib(ffi::atk_component_get_layer(self.to_glib_none().0))
        }
    }

    fn get_mdi_zorder(&self) -> i32 {
        unsafe {
            ffi::atk_component_get_mdi_zorder(self.to_glib_none().0)
        }
    }

    //fn get_position(&self, coord_type: /*Ignored*/CoordType) -> (i32, i32) {
    //    unsafe { TODO: call ffi::atk_component_get_position() }
    //}

    fn get_size(&self) -> (i32, i32) {
        unsafe {
            let mut width = mem::uninitialized();
            let mut height = mem::uninitialized();
            ffi::atk_component_get_size(self.to_glib_none().0, &mut width, &mut height);
            (width, height)
        }
    }

    fn grab_focus(&self) -> bool {
        unsafe {
            from_glib(ffi::atk_component_grab_focus(self.to_glib_none().0))
        }
    }

    //fn ref_accessible_at_point(&self, x: i32, y: i32, coord_type: /*Ignored*/CoordType) -> Option<Object> {
    //    unsafe { TODO: call ffi::atk_component_ref_accessible_at_point() }
    //}

    fn remove_focus_handler(&self, handler_id: u32) {
        unsafe {
            ffi::atk_component_remove_focus_handler(self.to_glib_none().0, handler_id);
        }
    }

    //fn set_extents(&self, x: i32, y: i32, width: i32, height: i32, coord_type: /*Ignored*/CoordType) -> bool {
    //    unsafe { TODO: call ffi::atk_component_set_extents() }
    //}

    //fn set_position(&self, x: i32, y: i32, coord_type: /*Ignored*/CoordType) -> bool {
    //    unsafe { TODO: call ffi::atk_component_set_position() }
    //}

    fn set_size(&self, width: i32, height: i32) -> bool {
        unsafe {
            from_glib(ffi::atk_component_set_size(self.to_glib_none().0, width, height))
        }
    }

    fn connect_bounds_changed<F: Fn(&Self, &Rectangle) + 'static>(&self, f: F) -> SignalHandlerId {
        unsafe {
            let f: Box_<Box_<Fn(&Self, &Rectangle) + 'static>> = Box_::new(Box_::new(f));
            connect(self.to_glib_none().0, "bounds-changed",
                transmute(bounds_changed_trampoline::<Self> as usize), Box_::into_raw(f) as *mut _)
        }
    }
}

unsafe extern "C" fn bounds_changed_trampoline<P>(this: *mut ffi::AtkComponent, arg1: *mut ffi::AtkRectangle, f: glib_ffi::gpointer)
where P: IsA<Component> {
    let f: &&(Fn(&P, &Rectangle) + 'static) = transmute(f);
    f(&Component::from_glib_borrow(this).downcast_unchecked(), &from_glib_borrow(arg1))
}
