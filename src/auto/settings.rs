...
impl Settings {
    ...
    //pub fn new_full<'a, 'b, P: Into<Option<&'a /*Ignored*/SettingsBackend>>, Q: Into<Option<&'b str>>>(schema: /*Ignored*/&SettingsSchema, backend: P, path: Q) -> Settings {
...
pub trait SettingsExt {
    ...
    //fn get_property_settings_schema(&self) -> /*Ignored*/Option<SettingsSchema>;

    //fn connect_change_event<Unsupported or ignored types>(&self, f: F) -> u64;

    fn connect_changed<F: Fn(&Self, &str) + 'static>(&self, f: F) -> u64;

    fn connect_writable_change_event<F: Fn(&Self, u32) -> Inhibit + 'static>(&self, f: F) -> u64;

    fn connect_writable_changed<F: Fn(&Self, &str) + 'static>(&self, f: F) -> u64;

    fn connect_property_backend_notify<F: Fn(&Self) + 'static>(&self, f: F) -> u64;

    fn connect_property_delay_apply_notify<F: Fn(&Self) + 'static>(&self, f: F) -> u64;

    fn connect_property_has_unapplied_notify<F: Fn(&Self) + 'static>(&self, f: F) -> u64;

    fn connect_property_path_notify<F: Fn(&Self) + 'static>(&self, f: F) -> u64;

    fn connect_property_schema_notify<F: Fn(&Self) + 'static>(&self, f: F) -> u64;

    fn connect_property_schema_id_notify<F: Fn(&Self) + 'static>(&self, f: F) -> u64;

    fn connect_property_settings_schema_notify<F: Fn(&Self) + 'static>(&self, f: F) -> u64;
}
...
impl<O: IsA<Settings> + IsA<glib::object::Object>> SettingsExt for O {
    ...
    //fn get_property_backend(&self) -> /*Ignored*/Option<SettingsBackend> {
    //    let mut value = Value::from(None::<&/*Ignored*/SettingsBackend>);
    //    unsafe {
    ...
    //fn get_property_settings_schema(&self) -> /*Ignored*/Option<SettingsSchema> {
    //    let mut value = Value::from(None::<&/*Ignored*/SettingsSchema>);
    //    unsafe {
...
where P: IsA<Settings> {
    ...
    f(&Settings::from_glib_none(this).downcast_unchecked(), &String::from_glib_none(key))
}
...
where P: IsA<Settings> {
    ...
    f(&Settings::from_glib_none(this).downcast_unchecked(), key).to_glib()
}
