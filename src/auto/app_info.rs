...
impl AppInfo {
    pub fn create_from_commandline<'a, T: Into<Option<&'a str>>>(commandline: &str, application_name: T, flags: AppInfoCreateFlags) -> Result<AppInfo, Error> {
        unsafe {
            let mut error = ptr::null_mut();
            let ret = ffi::g_app_info_create_from_commandline(commandline.to_glib_none().0, application_name.into().to_glib_none().0, flags.to_glib(), &mut error);
    ...
    pub fn launch_default_for_uri(uri: &str, launch_context: Option<&AppLaunchContext>) -> Result<(), Error> {
        unsafe {
            let mut error = ptr::null_mut();
            let _ = ffi::g_app_info_launch_default_for_uri(uri.to_glib_none().0, launch_context.to_glib_none().0, &mut error);
            if error.is_null() { Ok(()) } else { Err(from_glib_full(error)) }
        }
    }

    //#[cfg(feature = "v2_50")]
    //pub fn launch_default_for_uri_async(uri: &str, launch_context: &AppLaunchContext, cancellable: /*Ignored*/Option<&Cancellable>, callback: /*Unknown conversion*//*Unimplemented*/AsyncReadyCallback, user_data: /*Unimplemented*/Option<Fundamental: Pointer>) {
    //    unsafe { TODO: call ffi::g_app_info_launch_default_for_uri_async() }
    //}

    //#[cfg(feature = "v2_50")]
    //pub fn launch_default_for_uri_finish<T: IsA</*Ignored*/AsyncResult>>(result: &T) -> Result<(), Error> {
