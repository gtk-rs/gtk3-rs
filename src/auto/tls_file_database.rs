...
impl TlsFileDatabase {
    pub fn new<P: AsRef<std::path::Path>>(anchors: P) -> Result<TlsFileDatabase, Error> {
        unsafe {
            ...
            let ret = gio_sys::g_tls_file_database_new(anchors.as_ref().to_glib_none().0, &mut error);
...
impl<O: IsA<TlsFileDatabase>> TlsFileDatabaseExt for O {
    fn get_property_anchors(&self) -> Option<GString> {
        unsafe {
            ...
            gobject_sys::g_object_get_property(self.to_glib_none().0 as *mut gobject_sys::GObject, b"anchors\0".as_ptr() as *const _, value.to_glib_none_mut().0);
            value.get()
        }
    }
    ...
    fn set_property_anchors(&self, anchors: Option<&str>) {
        unsafe {
            gobject_sys::g_object_set_property(self.to_glib_none().0 as *mut gobject_sys::GObject, b"anchors\0".as_ptr() as *const _, Value::from(anchors).to_glib_none().0);
        }
    }
