...
impl<O: IsA<CharsetConverter>> CharsetConverterExt for O {
    fn get_num_fallbacks(&self) -> u32 {
        unsafe {
            gio_sys::g_charset_converter_get_num_fallbacks(self.as_ref().to_glib_none().0)
        }
    }

    fn get_use_fallback(&self) -> bool {
        unsafe {
            from_glib(gio_sys::g_charset_converter_get_use_fallback(self.as_ref().to_glib_none().0))
        }
    }

    fn set_use_fallback(&self, use_fallback: bool) {
        unsafe {
            gio_sys::g_charset_converter_set_use_fallback(self.as_ref().to_glib_none().0, use_fallback.to_glib());
        }
    }
    ...
    fn get_property_from_charset(&self) -> Option<GString> {
        unsafe {
            ...
            gobject_sys::g_object_get_property(self.to_glib_none().0 as *mut gobject_sys::GObject, b"from-charset\0".as_ptr() as *const _, value.to_glib_none_mut().0);
            value.get()
        }
    }
    ...
    fn get_property_to_charset(&self) -> Option<GString> {
        unsafe {
            let mut value = Value::from_type(<GString as StaticType>::static_type());
            gobject_sys::g_object_get_property(self.to_glib_none().0 as *mut gobject_sys::GObject, b"to-charset\0".as_ptr() as *const _, value.to_glib_none_mut().0);
            value.get()
        }
    }

    fn connect_property_use_fallback_notify<F: Fn(&Self) + 'static>(&self, f: F) -> SignalHandlerId {
        unsafe extern "C" fn notify_use_fallback_trampoline<P, F: Fn(&P) + 'static>(this: *mut gio_sys::GCharsetConverter, _param_spec: glib_sys::gpointer, f: glib_sys::gpointer)
