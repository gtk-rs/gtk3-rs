...
impl<O: IsA<Fontset>> FontsetExt for O {
    fn foreach<P: FnMut(&Fontset, &Font) -> bool>(&self, func: P) {
        ...
        unsafe extern "C" fn func_func<P: FnMut(&Fontset, &Font) -> bool>(fontset: *mut pango_sys::PangoFontset, font: *mut pango_sys::PangoFont, user_data: glib_sys::gpointer) -> glib_sys::gboolean {
        ...
        unsafe {
            pango_sys::pango_fontset_foreach(self.as_ref().to_glib_none().0, func, super_callback0 as *const _ as usize as *mut _);
        }
    }

    fn get_font(&self, wc: u32) -> Option<Font> {
        unsafe {
            from_glib_full(pango_sys::pango_fontset_get_font(self.as_ref().to_glib_none().0, wc))
        }
    }

    fn get_metrics(&self) -> Option<FontMetrics> {
        unsafe {
            from_glib_full(pango_sys::pango_fontset_get_metrics(self.as_ref().to_glib_none().0))
        }
    }
}
