...
glib_wrapper! {
    pub struct FileInputStream(Object<ffi::GFileInputStream, ffi::GFileInputStreamClass, FileInputStreamClass>) @extends InputStream, @implements Seekable;
...
impl<O: IsA<FileInputStream>> FileInputStreamExt for O {
    fn query_info<P: IsA<Cancellable>>(&self, attributes: &str, cancellable: Option<&P>) -> Result<FileInfo, Error> {
        unsafe {
            ...
            let ret = ffi::g_file_input_stream_query_info(self.as_ref().to_glib_none().0, attributes.to_glib_none().0, cancellable.map(|p| p.as_ref()).to_glib_none().0, &mut error);
    ...
    fn query_info_async<P: IsA<Cancellable>, Q: FnOnce(Result<FileInfo, Error>) + Send + 'static>(&self, attributes: &str, io_priority: glib::Priority, cancellable: Option<&P>, callback: Q) {
        ...
        unsafe {
            ffi::g_file_input_stream_query_info_async(self.as_ref().to_glib_none().0, attributes.to_glib_none().0, io_priority.to_glib(), cancellable.map(|p| p.as_ref()).to_glib_none().0, Some(callback), Box::into_raw(user_data) as *mut _);
        }
    }
