...
impl GLContext {
    ...
    #[cfg(feature = "v3_16")]
    pub fn set_property_shared_context(&self, shared_context: Option<&GLContext>) {
        unsafe {
            gobject_ffi::g_object_set_property(self.to_glib_none().0, "shared-context".to_glib_none().0, Value::from(shared_context).to_glib_none().0);
        }
    }

    #[cfg(feature = "v3_16")]
    pub fn set_property_window(&self, window: Option<&Window>) {
        unsafe {
            gobject_ffi::g_object_set_property(self.to_glib_none().0, "window".to_glib_none().0, Value::from(window).to_glib_none().0);
        }
    }
