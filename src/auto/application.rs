...
impl<O: IsA<Application> + IsA<Object>> ApplicationExt for O {
    ...
    //fn connect_open<Unsupported or ignored types>(&self, f: F) -> u64 {
    //    Ignored files: *.CArray TypeId { ns_id: 1, id: 149 }
    //}
...
unsafe extern "C" fn activate_trampoline<T>(this: *mut GApplication, f: gpointer)
...
unsafe extern "C" fn shutdown_trampoline<T>(this: *mut GApplication, f: gpointer)
where T: IsA<Application> {
    callback_guard!();
    let f: &Box_<Fn(&T) + 'static> = transmute(f);
    f(&Application::from_glib_none(this).downcast_unchecked())
...
unsafe extern "C" fn startup_trampoline<T>(this: *mut GApplication, f: gpointer)
where T: IsA<Application> {
    callback_guard!();
    let f: &Box_<Fn(&T) + 'static> = transmute(f);
    f(&Application::from_glib_none(this).downcast_unchecked())
