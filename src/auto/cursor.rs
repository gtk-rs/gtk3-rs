...
impl<O: IsA<Cursor> + IsA<glib::object::Object>> CursorExt for O {
    ...
    fn get_property_cursor_type(&self) -> CursorType {
        unsafe {
            ...
            gobject_ffi::g_value_init(value.to_glib_none_mut().0, <CursorType as StaticType>::static_type().to_glib());
    ...
    fn get_property_display(&self) -> Option<Display> {
        unsafe {
            let mut value = Value::uninitialized();
            gobject_ffi::g_value_init(value.to_glib_none_mut().0, <Display as StaticType>::static_type().to_glib());
