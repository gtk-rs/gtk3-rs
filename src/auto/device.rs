...
impl Device {
    ...
    pub fn grab<'a, P: Into<Option<&'a Cursor>>>(&self, window: &Window, grab_ownership: GrabOwnership, owner_events: bool, event_mask: EventMask, cursor: P, time_: u32) -> GrabStatus {
        let cursor = cursor.into();
        let cursor = cursor.to_glib_none().0;
        unsafe {
            from_glib(ffi::gdk_device_grab(self.to_glib_none().0, window.to_glib_none().0, grab_ownership.to_glib(), owner_events.to_glib(), event_mask.to_glib(), cursor, time_))
        }
    }
