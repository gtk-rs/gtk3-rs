...
impl ZlibCompressor {
    pub fn new(format: ZlibCompressorFormat, level: i32) -> ZlibCompressor {
        unsafe {
            from_glib_full(gio_sys::g_zlib_compressor_new(format.to_glib(), level))
        }
    }
}
...
impl<O: IsA<ZlibCompressor>> ZlibCompressorExt for O {
    fn get_file_info(&self) -> Option<FileInfo> {
        unsafe {
            from_glib_none(gio_sys::g_zlib_compressor_get_file_info(self.as_ref().to_glib_none().0))
        }
    }
    ...
    fn get_property_format(&self) -> ZlibCompressorFormat {
        unsafe {
            ...
            gobject_sys::g_object_get_property(self.to_glib_none().0 as *mut gobject_sys::GObject, b"format\0".as_ptr() as *const _, value.to_glib_none_mut().0);
            value.get().unwrap()
        }
    }
    ...
    fn connect_property_file_info_notify<F: Fn(&Self) + 'static>(&self, f: F) -> SignalHandlerId {
        unsafe extern "C" fn notify_file_info_trampoline<P, F: Fn(&P) + 'static>(this: *mut gio_sys::GZlibCompressor, _param_spec: glib_sys::gpointer, f: glib_sys::gpointer)
