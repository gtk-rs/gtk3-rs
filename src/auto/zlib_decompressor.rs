...
impl ZlibDecompressor {
    pub fn new(format: ZlibCompressorFormat) -> ZlibDecompressor {
        unsafe {
            from_glib_full(gio_sys::g_zlib_decompressor_new(format.to_glib()))
        }
    }
}
...
impl<O: IsA<ZlibDecompressor>> ZlibDecompressorExt for O {
    fn get_file_info(&self) -> Option<FileInfo> {
        unsafe {
            from_glib_none(gio_sys::g_zlib_decompressor_get_file_info(self.as_ref().to_glib_none().0))
        }
    }
    ...
    fn connect_property_file_info_notify<F: Fn(&Self) + 'static>(&self, f: F) -> SignalHandlerId {
        unsafe extern "C" fn notify_file_info_trampoline<P, F: Fn(&P) + 'static>(this: *mut gio_sys::GZlibDecompressor, _param_spec: glib_sys::gpointer, f: glib_sys::gpointer)
