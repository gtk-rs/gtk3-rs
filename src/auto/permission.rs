...
pub trait PermissionExt {
    ...
    fn acquire_async<P: Fn(Result<(), Error>) + Send + Sync>(&self, callback: P) -> Cancellable;
    ...
    fn release_async<P: Fn(Result<(), Error>) + Send + Sync>(&self, callback: P) -> Cancellable;
...
impl<O: IsA<Permission> + IsA<glib::object::Object>> PermissionExt for O {
    ...
    fn acquire_async<P: Fn(Result<(), Error>) + Send + Sync>(&self, callback: P) -> Cancellable {
        ...
        unsafe {
            ffi::g_permission_acquire_async(self.to_glib_none().0, cancellable.to_glib_none().0, Some(callback), Box::into_raw(user_data) as *mut _);
        }
        cancellable
    }
    ...
    fn release_async<P: Fn(Result<(), Error>) + Send + Sync>(&self, callback: P) -> Cancellable {
        ...
        unsafe {
            ffi::g_permission_release_async(self.to_glib_none().0, cancellable.to_glib_none().0, Some(callback), Box::into_raw(user_data) as *mut _);
        }
        cancellable
    }
