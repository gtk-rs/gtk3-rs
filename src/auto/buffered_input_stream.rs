...
impl BufferedInputStream {
    pub fn new<P: IsA<InputStream>>(base_stream: &P) -> BufferedInputStream {
        unsafe {
            InputStream::from_glib_full(gio_sys::g_buffered_input_stream_new(base_stream.as_ref().to_glib_none().0)).unsafe_cast()
        }
    }
...
pub trait BufferedInputStreamExt: 'static {
    fn fill<P: IsA<Cancellable>>(&self, count: isize, cancellable: Option<&P>) -> Result<isize, Error>;
...
impl<O: IsA<BufferedInputStream>> BufferedInputStreamExt for O {
    fn fill<P: IsA<Cancellable>>(&self, count: isize, cancellable: Option<&P>) -> Result<isize, Error> {
        unsafe {
            ...
            let ret = gio_sys::g_buffered_input_stream_fill(self.as_ref().to_glib_none().0, count, cancellable.map(|p| p.as_ref()).to_glib_none().0, &mut error);
    ...
    fn fill_async<P: IsA<Cancellable>, Q: FnOnce(Result<isize, Error>) + Send + 'static>(&self, count: isize, io_priority: glib::Priority, cancellable: Option<&P>, callback: Q) {
        ...
        unsafe extern "C" fn fill_async_trampoline<Q: FnOnce(Result<isize, Error>) + Send + 'static>(_source_object: *mut gobject_sys::GObject, res: *mut gio_sys::GAsyncResult, user_data: glib_sys::gpointer) {
            let mut error = ptr::null_mut();
            let ret = gio_sys::g_buffered_input_stream_fill_finish(_source_object as *mut _, res, &mut error);
        ...
        unsafe {
            gio_sys::g_buffered_input_stream_fill_async(self.as_ref().to_glib_none().0, count, io_priority.to_glib(), cancellable.map(|p| p.as_ref()).to_glib_none().0, Some(callback), Box::into_raw(user_data) as *mut _);
        }
    }
    ...
    fn get_available(&self) -> usize {
        unsafe {
            gio_sys::g_buffered_input_stream_get_available(self.as_ref().to_glib_none().0)
        }
    }

    fn get_buffer_size(&self) -> usize {
        unsafe {
            gio_sys::g_buffered_input_stream_get_buffer_size(self.as_ref().to_glib_none().0)
        }
    }
    ...
    fn peek_buffer(&self) -> Vec<u8> {
        unsafe {
            ...
            let ret = FromGlibContainer::from_glib_none_num(gio_sys::g_buffered_input_stream_peek_buffer(self.as_ref().to_glib_none().0, &mut count), count as usize);
            ret
        }
    }
    ...
    fn read_byte<P: IsA<Cancellable>>(&self, cancellable: Option<&P>) -> Result<i32, Error> {
        unsafe {
            let mut error = ptr::null_mut();
            let ret = gio_sys::g_buffered_input_stream_read_byte(self.as_ref().to_glib_none().0, cancellable.map(|p| p.as_ref()).to_glib_none().0, &mut error);
            if error.is_null() { Ok(ret) } else { Err(from_glib_full(error)) }
        }
    }
...
unsafe extern "C" fn notify_buffer_size_trampoline<P, F: Fn(&P) + 'static>(this: *mut gio_sys::GBufferedInputStream, _param_spec: glib_sys::gpointer, f: glib_sys::gpointer)
