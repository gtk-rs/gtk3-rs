...
impl Plane {
    ...
    pub fn new_from_point(normal: &Vec3, point: &Point3D) -> Plane {
        ...
        unsafe {
            let alloc = ffi::graphene_plane_alloc();
            ffi::graphene_plane_init_from_point(alloc, normal.to_glib_none().0, point.to_glib_none().0);
            from_glib_full(alloc)
        }
    }
    ...
    pub fn new_from_points(a: &Point3D, b: &Point3D, c: &Point3D) -> Plane {
        ...
        unsafe {
            let alloc = ffi::graphene_plane_alloc();
            ffi::graphene_plane_init_from_points(alloc, a.to_glib_none().0, b.to_glib_none().0, c.to_glib_none().0);
            from_glib_full(alloc)
        }
    }
